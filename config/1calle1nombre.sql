...
, results AS (
  SELECT
    ...
    , ccaa.id AS ccaa
    , municipios.name
    , sum(CASE WHEN (streets.way IS NOT NULL AND "streets.name:etymology:wikidata" IS NULL) THEN 1 ELSE 0 END) AS incomplete
    , sum(CASE WHEN (streets.way IS NOT NULL) THEN 1 ELSE 0 END) AS total
    , sum(CASE WHEN (streets.way IS NOT NULL AND "streets.name:etymology:wikidata" IS NULL) THEN st_length(streets.way, TRUE) ELSE 0 END) AS length_incomplete
